...
const WithdrawForm = ({ wallet, onBack }: WalletFormProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await apiRequest.post("/withdraw/wallet-proof", payload)
      ...
      if (response.status === 200) {
        ...
        // If receipt generation fails, we should ideally refund the stars
